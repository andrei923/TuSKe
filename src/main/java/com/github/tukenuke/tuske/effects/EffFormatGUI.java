...
import com.github.tukenuke.tuske.gui.GUI;
...
public class EffFormatGUI extends EffectSection {
	static {
		...
		Registry.newEffect(EffFormatGUI.class,
				...
				"(format|create|make) [a] gui slot [%-numbers%] of %players% with %itemstack% to [close then] (run|exe[cute]) %commandsender% command %string% [[(,| and)] (using|with) %-" + cr + "% [(button|click|action)]][[(,| and)] (using|with) cursor [item] %-itemstack%]",
